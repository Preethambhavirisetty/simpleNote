...
Standalone HTTP runtime for the LangGraph tool workflow:

`planner → executor → [summarizer] → fact_extractor → synthesizer → reviewer → [revision] → finalizer`

Approval interrupts only for destructive tools. The mid-loop **summarizer** compacts retained artifacts into `running_summary` when `enable_running_summary` is true.
...
Studio/runtime agents load `agents/default.yaml` and merge overrides from the runtime bundle (`active_version.config.runtime` or `runtime_overrides`). Use this to tune budgets per workload without changing the generic default:

```json
"runtime_overrides": {
  "policy": {
    "truncation": { "max_artifact_chars": 15000 }
  }
}
```

For a file-based Splunk agent, use `app/agent_workflow/agents/splunk.yaml`.

## Structured tool facts (MCP)

MCP tools can emit a top-level `facts` array (and/or `panels` / `items` collections) so `fact_extractor` preserves evidence without relying on summary line-splitting. Splunk `list_dashboards` and `get_dashboard` populate this contract.
