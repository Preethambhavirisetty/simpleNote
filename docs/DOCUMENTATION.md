...
**The Executor** is the workhorse. It works through the plan one step at a time. On each turn it looks at everything it knows — the plan, the evidence gathered so far, recent tool activity, reviewer feedback — and chooses exactly one action: search for tools, call a tool, or finish the current step. Every tool call is policy-checked, schema-validated, argument-injected, and executed under a deadline. Results are scored, summarized, and stored as *artifacts* — the evidence the rest of the pipeline runs on. Crucially, the executor only orchestrates tools; when the plan is done it hands off to the evidence-to-answer pipeline below rather than writing the answer itself. And when a follow-up question is too terse to search on, the planner has already rewritten it into a standalone retrieval query so the executor's tool discovery matches on real nouns rather than pronouns.
...
**The Summarizer** is a memory keeper that appears only on long runs. When gathered evidence piles up near its limit, the executor loop detours through it: in one model call it folds the least-important artifacts into a compact running summary, keeps the strongest ones intact, and hands control back to the executor with room to keep working. Nothing is lost — the summary is fed back into the model's context and into the facts the answer is built from, and a structured record of each folded artifact's source is kept so citations survive. The detour is optional and bounded by a hard cycle cap.

**The Fact Extractor** runs once the executor is done. It is deterministic — no model call — and its job is to distill the gathered artifacts into small, source-linked facts, preferring any structured facts the tools returned. From here on, the authoring nodes reason over these compact facts instead of raw tool output.

**The Synthesizer** is the single authoring pass. It writes one draft answer from the facts alone, tags it by kind (model prose, a mechanical dump, or a passed-through executor draft), and decides whether that draft even needs review — a clean, low-risk run can skip straight to finalization.

**The Reviewer** reads the draft answer against the plan's acceptance criteria and the gathered evidence, and returns a verdict: approve, revise, or reject. It is a judge only — it never rewrites the answer — and it accepts a verdict in JSON or markdown, falling back to a safe revise when a reply cannot be parsed. Before it accepts an approval, deterministic completion checks can veto an over-eager verdict. A revise verdict splits by cause: when the reviewer reports *missing evidence*, the run re-enters the executor to go gather it (an evidence-revise, bounded by an explore-cycle cap); when the draft is merely worded poorly but the facts suffice, it goes to a single bounded text-revision pass. A reject returns the best available draft (routing back to the planner is a configured but not-yet-wired option, so reject behaves like abort today). The reviewer can run in an economical mode where it only engages when something actually went wrong during the run — a failed tool call, a denial, an error — and clean runs skip the review entirely. Its cycles are capped, and when the cap is reached the run ends honestly with the best available answer and an explanatory error.

**The Reviser** rewrites the draft at most once, using the same facts and the reviewer's concrete issues. It calls no tools and has no path back to the executor: it handles only the *text-revise* case — improving wording and grounding from the evidence already gathered. The other case, where the reviewer finds evidence genuinely missing, is handled by re-entering the executor directly (the bounded evidence-revise path), so gathering more data and rewording are kept as distinct, separately-bounded loops rather than one unbounded spiral.
...
Sixth, distilling, drafting, and review. When the steps are done, the run does not jump straight to prose. A deterministic fact extractor first distills the artifacts into small, source-linked facts; the synthesizer then writes one draft answer from those facts. The reviewer weighs the draft against the acceptance criteria. If it finds only wording problems, a single bounded revision pass rewrites it from the same facts. If instead it finds evidence genuinely missing — say the SLA numbers were never actually retrieved — it sends the run back to the executor to gather more, a bounded evidence-revise that then flows normally back through extraction, synthesis, and review. (If, along the way, the gathered evidence had grown large, the run would have quietly detoured through the summarizer to compact its memory and keep going.)
...
**Raw results stay out of state.** The truncator keeps a readable summary and a compact structural reference — counts, key names, identifiers, a truncation flag — instead of the raw payload. The reference is enough for a host application to re-fetch or audit the original if it needs to; the run itself never carries the weight. Crucially, truncation only trims to *fit*: a string field is kept whole and clipped only when the entire result exceeds its budget, so a small result whose one field happens to be long is never needlessly cut.

**One rich result is never lost to compression.** Compressing tool results into small facts is ideal for many-source runs but lossy for a single rich answer — a report shredded into fact lines loses content. So the writer sees both the top tool-result summaries *verbatim* (bounded, highest-scored first) and the compact facts as a provenance index over them. For a single rich result its whole summary survives into the answer; for many results the strongest few are verbatim and the facts cover the tail. The per-fact and total-facts budgets are derived from the context window so the combined prompt stays in bounds.
...
**Large evidence is compacted, not dropped.** On a long run, when retained artifacts approach their cap, the executor loop detours through a summarizer that folds the least-important artifacts into a compact running memo in one model call, keeps the strongest artifacts intact, and returns with freed context. The memo is injected back into the model's context and seeded into the facts the answer is built from, and a structured provenance record of each folded artifact is kept so citations survive the compaction. The detour is optional and bounded by a hard cycle cap, so it can free memory repeatedly without ever spinning.

**Structure is preserved through compression.** When a tool returns structured data — a list of facts, rows, panels, or a table — truncation keeps a bounded copy of it rather than reducing it to a count, and the fact extractor reads it directly. Returning large collections under these recognized shapes is how an application gets rich, faithful extraction instead of line-split summaries.
...
- **A policy section** — every knob that shapes behavior: iteration and review-cycle limits, the bounded re-exploration cap, the no-progress stop (how many unproductive turns before the run stops early, and the score that counts as progress), per-step tool caps, the context token budget, model and tool deadlines, truncation budgets (including the per-fact cap), retention caps for artifacts and history, the list of destructive tools, whether the fast path is allowed, whether the final answer is rendered, whether grounding is enforced, whether the planner and running-summary compaction run, the reviewer's mode (always, or only on risk), tool allow/deny/required lists, and argument injection mappings.
- **A resources section** — the shared checkpoint store and tool index described above.

**Exploration profiles** sit on top of this. Rather than tune every knob by hand, a request can select a `quick` or `heavy` mode — via a field in its trusted runtime context, or a service-wide environment default. Quick trades depth for latency (fast path on, no re-exploration, review only on risk); heavy trades latency for depth (planner and always-on review, higher iteration / re-exploration / no-progress caps, larger truncation and answer-length budgets). The mode is a preset layered between the base config and the caller's explicit overrides — the mode sets the baseline, an explicit override still wins — so one deployed service can present a snappy assistant to one call and a deep research agent to the next, each isolated by its own config signature.
...
- Bundle variants of both accept a runtime bundle instead of a configuration reference. The streamed metadata event reports the resolved exploration profile.
- A **resume** endpoint takes a thread identifier and an approve-or-deny decision, and continues a paused run to completion.
- An **actions** endpoint runs one isolated pipeline stage per call — for manual or Postman testing. It can load and inspect a configuration (secrets redacted), validate a request, build initial state, check the fast path, resolve or apply an exploration profile, parse planner or reviewer markdown, exercise truncation and scoring, evaluate a routing decision, or run any single node against scripted mock providers. It is how you verify one stage's behavior in isolation without driving the whole graph.
...
- **Deterministic fact extraction and a single synthesis pass** compress evidence with no model call and write the answer in exactly one, instead of drafting inside the tool loop.
- **In-loop memory compaction** trades one summarizer call for freed context so long investigations stay in budget instead of dropping evidence.
- **The stagnation early stop** ends a run as soon as tool work stops producing useful new evidence, rather than burning the whole iteration budget — a generic, score-based check with no application heuristics.
...
With everything enabled, a clean trivial message costs one model call; a clean single-step tool run costs roughly three (one tool-selection turn, one finish turn, one synthesis pass — fact extraction is deterministic and free, and the finalizer reuses the synthesized prose without another call); and each additional plan step, or a review-plus-revision, adds roughly one to two calls.
...
Today, the service can: answer trivial messages instantly; plan multi-step work; rewrite terse follow-up questions into standalone search queries so tool discovery still matches; discover tools dynamically across multiple MCP servers, by catalog or by semantic search; call tools with schema validation, policy enforcement, and trusted argument injection; pause destructive actions for human approval and resume them on any worker, surviving restarts with a durable store; distil gathered evidence into compact source-linked facts and compact its own working memory mid-run when evidence grows large, preserving provenance so citations survive; review its own drafts and revise within a bounded cycle; render grounded, streamed final answers that are identical whether the caller used the synchronous or streaming endpoint; retry transient failures intelligently; account for every token spent; and serve many tenants from one process with credential-isolated caching.

Its known limits, stated plainly: execution is synchronous — each active run occupies a worker thread, which is comfortable at tens of concurrent runs per instance but is the ceiling to plan around, and true mid-flight cancellation of an in-progress model call is not possible until an async migration lands. Memory compaction is within-run only — the summarizer frees context during a single run, but there is no built-in semantic long-term memory across sessions; conversation history is whatever the caller passes in, and a cross-session memory provider fed by the shared resources section is the designed next step. Observability data is complete but not yet exported to any metrics backend. And configuration versioning is content-addressed, not history-tracked — rollback is a job for version control.
...
| Max loop/context guards | **Yes** | Executor turn caps, review/revision/explore cycle caps, per-step tool caps, a search loop breaker, a graph recursion limit, token-budgeted context packing, and per-call deadlines. |
| Progress-based early stop | **Yes** | A generic, score-based stagnation stop ends exploration once tool work stops adding useful new evidence — turning the hard caps into an efficiency stop, with no app-specific heuristics. Configurable and disable-able. |
| Exploration modes (quick / heavy) | **Yes** | A per-request preset reshapes latency-vs-depth without new deployments — selectable from runtime context, a UI, or an env default. Layered as base < profile < explicit override, isolated by config signature. |
| Lossless small/rich results | **Yes** | Truncation trims only to fit budget (small results survive whole); the synthesizer reads top tool-result summaries verbatim plus compact facts, so a single rich result is not lost to fact compression. |
| Per-stage testing (action endpoint) | **Yes** | An `/actions` endpoint runs one isolated stage (config, parsing, truncation, routing, or any single node) against scripted mock providers, for manual/Postman verification without the full graph. |
| In-run memory compaction | **Yes, optional** | A bounded summarizer node folds low-value artifacts into a running memo mid-loop to free context, keeps the strongest artifacts, and preserves structured provenance so citations survive. Cross-session long-term memory remains the caller's responsibility. |
| Follow-up query rewrite | **Yes** | The planner rewrites terse, pronoun-heavy follow-ups into standalone retrieval queries (with a deterministic fallback when the planner is off), so semantic tool search matches on real nouns without an extra model call. |
| Evidence-to-answer separation | **Yes** | The executor only gathers; a deterministic fact extractor compresses evidence, a single synthesizer writes the answer, the reviewer judges without rewriting, and a bounded revision node handles one text rewrite. |
| Reviewer-driven re-exploration | **Yes, bounded** | When the reviewer reports missing evidence, the run re-enters the executor to gather more tools (evidence-revise) rather than rewriting from the same facts — capped by an explore-cycle limit and the review-cycle limit, with feedback passed to guide the next tool choice. Wording-only revises still go to the revision node. |
