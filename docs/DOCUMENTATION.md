...
| Evidence-to-answer separation | **Yes** | The executor only gathers; a deterministic fact extractor compresses evidence, a single synthesizer writes the answer, the reviewer judges without rewriting, and a bounded revision node handles one rewrite — no review-execute loop. |
