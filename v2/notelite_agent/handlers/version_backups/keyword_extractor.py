...
def _refine_with_pos(term: str, noun_set: set[str]) -> str | None:
    ...
    # If no words remain or the sole remaining word is not a noun, discard.
    if not words or (len(words) == 1 and words[0] not in noun_set):
        return None
...
def _is_subphrase(a: str, b: str) -> bool:
    """Return True if every token in `a` appears in `b` in the same relative order."""
    a_words = a.split()
    b_words = b.split()
    it = iter(b_words)
    return all(word in it for word in a_words)
...
def prune_keywords(keywords: list[str]) -> list[str]:
    ...
    selected: list[str] = []
    for kw in keywords:
        # Check if kw is already covered by something in selected
        if any(_is_subphrase(kw, existing) for existing in selected):
            continue

        # Remove all existing entries that are subsumed by the new, longer kw
        selected = [e for e in selected if not _is_subphrase(e, kw)]
        selected.append(kw)
...
# text4 = """
# Test Case: Strategic Alignment of Various Elements and Other Bits

# Introduction
# In this summary, we will look at a variety of aspects regarding the general activity of our team. This conversation has been happening for a few weeks now, and we have identified several factors that contribute to our current situation. It is not just about one thing; it is about the intersection of various activities and the conversations we have during the day and at night.


# Core Elements of the Activity
# When we consider the strategic stuff, we often find that certain bits of information are missing. For instance, that night when we reviewed the quarterly results, we noticed a few instances where the activity logs didn't match the actual output. This time, we are focusing on the operational things that matter. If you look at the various activities involved in project management, you will see that anything can become a major factor if it isn't handled correctly.


# We’ve had many conversations about the variety of stuff we handle. Some people think it’s just about the technical bits, but it’s also about the social things. During the day, we focus on the main activity, but at night, we reflect on the different aspects of the work. This conversation is essential for our growth.


# A Few More Points to Consider
# There are always something new to discuss when it comes to coordination stuff. Last night, I was thinking about the conversation we had regarding the alignment things. It’s not just about one element; it’s about all the elements combined. For instance, if you have a few factors that aren't aligned, you’ll spend a lot of time fixing the resulting activities. This time, I recommend looking at the integration bits. Integration is the thing that will save us time.


# Anything can happen during the implementation phase. That night, we saw a few errors that didn't mean anything, but over time, they became a significant thing. We spent the whole day looking at the activity reports, trying to find something—anything—that would explain the variety of results.


# Conclusion
# To wrap up this conversation, I hope this summary provided a few insights into the stuff we do. Management is a complex activity, and there is always something to improve. Next time, we will talk about more specific strategic things and how to manage the bits and pieces of the organization during the day. It’s been a long night of planning, but I think we’ve covered a lot of things.
# """

# Keywords_1 = ['core elements', 'test case', 'strategic alignment', 'variety', 'report', 'implementation phase', 'aspect', 'integration', 'coordination', 'project management', 'output', 'log', 'situation', 'summary', 'team', 'information', 'intersection', 'element']

# Keywords_2 = ['core elements', 'test case', 'strategic alignment', 'variety', 'team', 'factors', 'element', 'summary', 'situation', 'aspects', 'logs', 'results', 'report', 'implementation phase', 'aspect', 'integration', 'coordination', 'project management', 'output', 'log']

# Keywords_3 = ['core elements', 'test case', 'strategic alignment', 'variety', 'report', 'implementation phase', 'aspect', 'integration', 'coordination', 'project management', 'output', 'log', 'situation', 'summary', 'team', 'information', 'intersection', 'element']

# I'm testing my keyword extractor logic for a RAG pipeline for a blog like app during chunking and including these keywords in metadata