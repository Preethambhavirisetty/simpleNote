...
# APPENDIX_RE = re.compile(r"^#{1,6}\s+appendix\b|(?:^|\n)appendix\b", re.IGNORECASE)
# FAQ_RE = re.compile(
#     r"^#{1,6}\s+(?:faq|frequently asked questions)\b|(?:^|\n)Q:\s+",
#     re.IGNORECASE,
# )
# TRANSCRIPT_HEADING_RE = re.compile(r"^#{1,6}\s+.*transcript\b", re.IGNORECASE)
# QUOTE_HEADING_RE = re.compile(r"^#{1,6}\s+quotes?\b", re.IGNORECASE)
# JSON_RE = re.compile(
#     r"^```json\b|^\s*[\{\[][\s\S]*[\}\]]\s*$",
#     re.IGNORECASE,
# )
# RAW_CODE_RE = re.compile(
#     r"(?m)^\s*(?:def\s+|class\s+|import\s+|from\s+\S+\s+import\s+|function\s+|const\s+|let\s+|var\s+|return\b|if\s*\(|for\s*\(|while\s*\()"
# )
# STRUCTURED_LIST_RE = re.compile(
#     r"(?m)^\s*(?:\d+(?:\.\d+)*[.)]?|[A-Z]\.|\([ivxlcdm]+\))\s+\S",
#     re.IGNORECASE,
# )
# CODE_LABELS = {"def", "class", "if", "for", "while", "function", "return"}
# QUOTE_PAIRS = (('"', '"'), ("'", "'"), ("“", "”"), ("‘", "’"))
...
#     return is_heading_only_chunk(text) or HEADING_RE.sub("", text).strip() == ""


# def is_fenced_json_type(text: str, body: str) -> bool:
#     return is_fenced_code_block(text) and bool(JSON_RE.search(text))


# def is_fenced_code_type(text: str, body: str) -> bool:
#     return is_fenced_code_block(text)


# def is_json_type(text: str, body: str) -> bool:
#     return bool(JSON_RE.search(text))


# def is_raw_code_type(text: str, body: str) -> bool:
#     return bool(RAW_CODE_RE.search(text))


# def is_faq_type(text: str, body: str) -> bool:
#     return bool(FAQ_RE.search(text))
...
#     speaker_lines = sum(1 for line in lines if is_speaker_label(line))
#     if speaker_lines >= 2:
#         return True
#     return speaker_lines == 1 and len(lines) >= 2


# def starts_transcript_block(text: str) -> bool:
#     lines = _nonempty_lines(text)
#     return bool(lines) and is_speaker_label(lines[0]) and len(lines) >= 2
...
#         return False
#     if re.fullmatch(r"[A-Z]{1,10}", label):
#         return False
#     words = label.split()
#     first_word = words[0].lower() if words else ""
#     if first_word in CODE_LABELS:
#         return False
#     return 1 <= len(words) <= 6 and any(char.isalpha() for char in label)


# def is_glossary_type(text: str, body: str) -> bool:
#     if GLOSSARY_HEADING_RE.search(text):
#         return True
#     return starts_glossary_block(text)
...
#     if not lines:
#         return False
#     term_lines = sum(1 for line in lines if GLOSSARY_TERM_RE.match(line))
#     return term_lines >= 1 and term_lines >= max(1, len(lines) - 1)
...
#     if QUOTE_HEADING_RE.search(text):
#         return True
#     if not lines:
#         return False

#     blockquote_lines = [line for line in lines if line.startswith(">")]
#     if blockquote_lines and len(blockquote_lines) == len(lines):
#         return True

#     wrapped_lines = [line for line in lines if _is_wrapped_quote(line)]
#     if wrapped_lines and len(wrapped_lines) == len(lines):
#         return True

#     compact = "\n".join(lines)
#     return len(lines) > 1 and _is_wrapped_quote(compact)


# def is_table_type(text: str, body: str) -> bool:
#     return is_table_like(text)


# def is_address_type(text: str, body: str) -> bool:
#     return is_address_like_chunk(text)


# def is_contact_type(text: str, body: str) -> bool:
#     return is_contact_like_chunk(text)


# def is_structured_list_type(text: str, body: str) -> bool:
#     return is_structured_list_like(body)


# def is_structured_list_like(text: str) -> bool:
#     lines = _nonempty_lines(text)
#     if len(lines) < 2:
#         return False
#     list_lines = sum(1 for line in lines if STRUCTURED_LIST_RE.match(line))
#     return list_lines >= max(2, len(lines) - 1)


# def continues_structured_list(previous: str, current: str) -> bool:
#     return is_structured_list_like(f"{previous}\n{current}")


# def is_list_type(text: str, body: str) -> bool:
#     return is_list_chunk(body)
...
#         return False
#     return any(
#         stripped.startswith(open_quote) and stripped.endswith(close_quote)
...
# ---------------------------------------------------------------------------
# Type detectors
# Each takes (full_text: str, body: str) -> bool
# body = text with leading heading stripped (via without_leading_heading)
# ---------------------------------------------------------------------------
