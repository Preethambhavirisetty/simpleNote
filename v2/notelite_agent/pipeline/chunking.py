"""Text splitting and postprocessing for note ingestion.

Three-tier splitting strategy:
    1. Paragraph boundaries (double newlines)
    2. Heading detection (short capitalized lines)
    3. Semantic splitting (LlamaIndex SemanticSplitterNodeParser)
    4. Hard window split as final size enforcement

All chunks are postprocessed for quality: heading merging,
list linking, table/address consolidation, and noise removal.
"""
...
def _semantic_split(text):
    """Fall back to semantic splitting for large unstructured text."""
...
# ── Chunk classification and validation ──────────────────────────────────
...
def _postprocess_chunks(chunks):
    """Final cleanup: remove dividers, merge orphan headings, enforce max size."""
    cleaned_chunks = []
    for chunk in chunks:
...
def split_into_sections(text):
    ...
    log.info("Generated %d chunks (original=%d chars, chunked=%d chars)",
             len(chunks), len(text), sum(len(c) for c in chunks))
    return chunks
